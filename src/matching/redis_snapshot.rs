...
use crate::matching::order_book::Snapshot;
...
impl RedisSnapshotStore {
    pub async fn new_redis_snapshot_store(product_id: &str, ip: &str, port: u16) -> Result<RedisSnapshotStore, CustomError> {
    ...
    pub async fn get_latest(&mut self) -> Result<Option<Snapshot>, CustomError> {
        return match self.redis_client.get(
            &*format!("{}{}", TOPIC_SNAPSHOT_PREFIX, self.product_id)
        ).await {
            Ok(s) => {
                match s {
